...
<head>
    ...
    <title>CM Commission Report</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .hero-header::after {
            ...
            top: -40%; right: -5%; width: 45%; height: 200%;
...
<body>
    ...
    <div class="sidebar" style="background-color:#2c3e50; padding-left:20px; padding-top:2px;">
        ...
        <a href="/search">        <i class="fas fa-search-dollar fa-lg" style="color:#e67e22; margin-right:8px;"></i> BM + CM + CIR</a>
        <a href="/stop-payments"> <i class="fas fa-hand-paper fa-lg"    style="color:#c0392b; margin-right:8px;"></i> Stop Payment</a>
        <a href="/recoveries">    <i class="fas fa-wallet fa-lg"         style="color:#2ecc71; margin-right:8px;"></i> Recovery Report</a>
        <a href="/commission">    <i class="fas fa-chart-line fa-lg"     style="color:#e74c3c; margin-right:8px;"></i> CM Commission</a>
        <a href="/import" style="margin-left:-3px"><i class="fas fa-balance-scale fa-lg" style="color:#f1c40f; margin-right:10px;"></i>CIR Commission</a>
        <a href="/import-export"> <i class="fas fa-user-tie fa-lg"       style="color:#3498db; margin-right:8px;"></i> BM Commission</a>
        <a href="/google-index" style="margin-left:-2px"><i class="fas fa-user-friends fa-lg" style="color:#8e44ad; margin-right:10px;"></i>Broker Register</a>
        <a href="/coming1">       <i class="fas fa-sliders-h fa-lg"      style="color:#f39c12; margin-right:8px;"></i> Control Panel</a>
        <a href="/login">         <i class="fas fa-sign-out-alt fa-lg"   style="color:#16a085; margin-right:8px;"></i> Logout</a>
    </div>
    ...
    <div class="main-content">
            ...
            <div class="hero-inner">
                ...
                <div class="hero-left">
                    ...
                    <div class="hero-text">
                        ...
                        <h1 class="hero-title">CM&nbsp;Commission&nbsp;Report</h1>
                        <span class="hero-subtitle">
                            @if(request('search'))
                                Filtered by &nbsp;<strong style="color:rgba(255,255,255,0.7)">"{{ request('search') }}"</strong>
                                &nbsp;·&nbsp; {{ $commissions->total() }} record(s)
                            @else
                                {{ $commissions->total() }} total records
                ...
                <div class="hero-right">
                    <form method="GET" action="{{ url()->current() }}" id="searchForm" class="hero-search-form">
                        <div class="search-pill">
                            ...
                            <input
                                ...
                                placeholder="AGT Code, PAN Number or Ref Code…"
                                autocomplete="off"
                            >
                            @if(request('search'))
                                <button type="button" class="btn-clear-x" onclick="window.location='{{ url()->current() }}'" title="Clear">
                    ...
                    <div class="d-flex align-items-center justify-content-between w-100">
                        <div class="search-hints">
                            <span>Search by:</span>
                            <span class="hint-tag">AGT Code</span>
                            <span class="hint-tag">PAN Number</span>
                            <span class="hint-tag">Ref Code</span>
                        </div>
        ...
        @if(request('search'))
            <div class="mb-3">
                <span class="result-badge">
                    <span class="dot"></span>
                    Showing <strong>{{ $commissions->total() }}</strong> result(s) for
        ...
        <div class="table-container">
            <div class="table-scroll">
                    ...
                    <table class="table table-bordered" id="brokerTable">
                        <thead>
                            <tr>
                                ...
                                <th>DO NAME</th>
                                <th>REFERENCE ID</th>
                                <th class="{{ request('search') ? 'hl' : '' }}">AGT CODE</th>
                                <th>CODE</th>
                                <th>NAME</th>
                                <th>VCHR NO</th>
                                <th>CREATED DATE</th>
                                ...
                                <th>BANK NAME</th>
                                <th>IFSC</th>
                                <th class="{{ request('search') ? 'hl' : '' }}">PAN NUMBER</th>
                                <th class="{{ request('search') ? 'hl' : '' }}">REF CODE</th>
                                <th>REMARK 1</th>
                                <th>REMARK 2</th>
                                <th>REMARK 3</th>
                            </tr>
                        ...
                        <tbody>
                            @foreach ($commissions as $commission)
                                <tr>
                                    ...
                                    <td class="{{ request('search') ? 'hl' : '' }}">{{ $commission->agt_code }}</td>
                                    ...
                                    <td class="{{ request('search') ? 'hl' : '' }}">{{ $commission->pan_number }}</td>
                                    <td class="{{ request('search') ? 'hl' : '' }}">{{ $commission->ref_code }}</td>
                ...
                @else
                    <div class="no-results">
                        ...
                        <small style="font-size:13px;">Try a different AGT Code, PAN Number, or Ref Code.</small><br><br>
            ...
            <div class="pag-wrap">
                <div class="pag-info">
                    Showing <strong>{{ $commissions->firstItem() }}</strong>
                    to <strong>{{ $commissions->lastItem() }}</strong>
                    of <strong>{{ $commissions->total() }}</strong> results
                    @if(request('search'))
                        &nbsp;·&nbsp; filtered by <strong>"{{ request('search') }}"</strong>
                    @endif
                </div>
                {{ $commissions->appends(request()->only('search'))->links('pagination::bootstrap-5') }}
            </div>
        </div>
...
{{--
=====================================================================
  FinalCMCommissionController.php — update the index() method:
=====================================================================
