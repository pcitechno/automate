...
<body>
    ...
    <div class="main-content">
        <div class="container">
            <h1 class="colorful-text">CM-COMMISSION</h1>
            <div class="mb-3">
                    ...
                    @foreach ($commissions as $commission)
                        @php
                            ...
                            if (!isset($groupedData[$key])) {
                                $groupedData[$key] = [
                                    'so_code' => $commission->do_name,
                                    'do' => $commission->csc_name,
                                    'so_name' => $commission->pan_number,
                                    'trainer_id' => $commission->agt_code,
                            ...
                            $groupedData[$key]['details'][] = [
                                'final_amount' => $commission->remark_3,
                                'bank_name' => $commission->amount_payable,
                                'bank_ac' => $commission->tds_amount,
                                'ifsc_code' => $commission->final_payable_amount,
                                'cir_no' => $commission->clearing_date,
                                'date' => $commission->bank_name,
                                'bank_ac_no' => $commission->account_no,
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($group['details'] as $detail)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ number_format((float) $detail['bank_name'], 2) }}</td>
                                                <td>{{ number_format((float) $detail['bank_ac'], 2) }}</td>
                                                <td>{{ number_format((float) $detail['ifsc_code'], 2) }}</td>
                                                <td>{{ $detail['cir_no'] }}</td>
                                                <td>{{ $detail['date'] }}</td>
                                            ...
                                            $totalPayable += is_numeric($detail['bank_name']) ? (float)$detail['bank_name'] : 0;
                                            $totalTds += is_numeric($detail['bank_ac']) ? (float)$detail['bank_ac'] : 0;
                                            $totalFinal += is_numeric($detail['ifsc_code']) ? (float)$detail['ifsc_code'] : 0;
                                            @endphp
                                        @endforeach
